...
class PaketController extends Controller
{
    ...
    public function halamanPaket()
    {
        ...
        ->count();
    	$paket_kilos = Paketkilo::all();
    	$paket_satus = Paketsatu::all();
    ...
    {
        $outlets = Outlet::all();
    	$max = Paketkilo::max('kd_paket');
        $check_maks = Paketkilo::select('paketkilos.kd_paket')
        ->count();
        if($check_maks == null){
            $max_code = "PK001";
        }else{
    ...
    {
        $outlets = Outlet::all();
    	$max = Paketsatu::max('kd_barang');
        $check_maks = Paketsatu::select('paketkilos.kd_barang')
        ->count();
        if($check_maks == null){
            $max_code = "PS001";
        }else{
    ...
    {
        $paket_kilos = Paketkilo::join('outlets', 'outlets.id', '=', 'paketkilos.id_outlet')
        ->select('paketkilos.*', 'outlets.nama as nama_outlet')
        ->where('paketkilos.id', $id)
        ->first();
    ...
    {
        $paket_satus = Paketsatu::join('outlets', 'outlets.id', '=', 'paketsatus.id_outlet')
