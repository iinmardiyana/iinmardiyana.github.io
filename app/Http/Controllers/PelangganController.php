...
class PelangganController extends Controller
{
    ...
    public function detailPelangganNonMember($id)
    {
        ...
        ->count();
        if($check_kilo != null){
            $checkout_kilos = Checkout_kilo::join('paketkilos', 'paketkilos.kd_paket', '=', 'checkout_kilos.kd_paket')
            ->select('checkout_kilos.*', 'paketkilos.nama_paket', 'paketkilos.harga_paket as harga_paket_satuan')
            ->where('kd_invoice', $transaksis->kd_invoice)
            ->get();
            $checkout_kiloan = Checkout_kilo::join('paketkilos', 'paketkilos.kd_paket', '=', 'checkout_kilos.kd_paket')
            ->select('checkout_kilos.*', 'paketkilos.nama_paket', 'paketkilos.harga_paket as harga_paket_satuan', 'paketkilos.antar_jemput_paket')
            ->where('kd_invoice', $transaksis->kd_invoice)
            ->first();
            $checkout_satuan = "";
            $checkout_satus = "";
            ...
            $checkout_satus = Checkout_satu::join('paketsatus', 'paketsatus.kd_barang', '=', 'checkout_satus.kd_barang')
            ->select('checkout_satus.*', 'paketsatus.nama_barang', 'paketsatus.ket_barang', 'paketsatus.harga_barang as harga_barang_satuan')
            ->where('kd_invoice', $transaksis->kd_invoice)
            ->get();
            $checkout_satuan = Checkout_satu::join('paketsatus', 'paketsatus.kd_barang', '=', 'checkout_satus.kd_barang')
            ->select('checkout_satus.*', 'paketsatus.nama_barang', 'paketsatus.ket_barang', 'paketsatus.harga_barang as harga_barang_satuan')
            ->where('kd_invoice', $transaksis->kd_invoice)
            ->first();
            $checkout_kiloan = "";
    ...
    public function pdfPelanggan($id)
    {
        ...
        if($check_kilo != 0){
            ...
            ->get();
            $checkout_kilo = Checkout_kilo::join('paketkilos', 'paketkilos.kd_paket', '=', 'checkout_kilos.kd_paket')
            ->select('checkout_kilos.*', 'paketkilos.nama_paket', 'paketkilos.antar_jemput_paket', 'paketkilos.harga_paket as harga_paket_satuan')
            ->where('checkout_kilos.kd_invoice', $transaksis->kd_invoice)
            ->first();
            $checkout_satus = "";
            $checkout_satu = "";
            ...
            ->get();
            $checkout_satu = Checkout_satu::join('paketsatus', 'paketsatus.kd_barang', '=', 'checkout_satus.kd_barang')
            ->select('checkout_satus.*', 'paketsatus.nama_barang', 'paketsatus.ket_barang', 'paketsatus.harga_barang as harga_barang_satuan')
            ->where('checkout_satus.kd_invoice', $transaksis->kd_invoice)
            ->first();
            $checkout_kilos = "";
    ...
    public function halamanLayananMember($id)
    {
        ...
        ->where('kd_pengguna', $pelanggans->kd_pelanggan)
        ->first();
        if($pelanggans->cek_level == 'member')
        {
    ...
    {
        $paketkilos = Paketkilo::select('paketkilos.*')
        ->where('paketkilos.id_outlet', $id)
        ->get();
        return view('halaman_pelanggan.sort_outlet_tabel_kiloan', compact('paketkilos'));
    }

    // Sortir Outlet Tabel Satuan
    public function sortOutletTabelSatuan($id)
    {
        $paketsatus = Paketsatu::select('paketsatus.*')
        ->where('paket_satus.id_outlet', $id)
        ->get();
        return view('halaman_pelanggan.sort_outlet_tabel_satuan', compact('paketsatus'));
    }
    ...
    public function lihatPaketKiloMember($id)
    {
        ...
        ->first();
        $checkout_kilos = Checkout_kilo::join('paketkilos', 'paketkilos.kd_paket', '=', 'checkout_kilos.kd_paket')
        ->select('checkout_kilos.id', 'checkout_kilos.kd_invoice', 'checkout_kilos.kd_paket', 'checkout_kilos.berat_barang', 'checkout_kilos.metode_pembayaran', 'checkout_kilos.harga_paket as harga_paket_total', 'checkout_kilos.harga_antar', 'checkout_kilos.harga_total', 'paketkilos.nama_paket', 'paketkilos.harga_paket as harga_paket_satuan')
        ->where('kd_invoice', $transaksis->kd_invoice)
        ->get();
        $checkout_kilo = Checkout_kilo::join('paketkilos', 'paketkilos.kd_paket', '=', 'checkout_kilos.kd_paket')
        ->select('checkout_kilos.id', 'checkout_kilos.kd_invoice', 'checkout_kilos.kd_paket', 'checkout_kilos.berat_barang', 'checkout_kilos.metode_pembayaran', 'checkout_kilos.harga_paket as harga_paket_total', 'checkout_kilos.harga_antar', 'checkout_kilos.harga_total', 'paketkilos.nama_paket', 'paketkilos.antar_jemput_paket')
        ->where('kd_invoice', $transaksis->kd_invoice)
        ->first();
    ...
    public function lihatPaketSatuMember($id)
    {
        ...
        ->first();
        $checkout_satus = Checkout_satu::join('paketsatus', 'paketsatus.kd_barang', '=', 'checkout_satus.kd_barang')
        ->select('checkout_satus.id', 'checkout_satus.kd_invoice', 'checkout_satus.kd_barang', 'checkout_satus.jumlah_barang', 'checkout_satus.metode_pembayaran', 'checkout_satus.harga_barang as harga_barang_total', 'checkout_satus.harga_antar', 'checkout_satus.harga_total', 'paketsatus.nama_barang', 'paketsatus.harga_barang as harga_barang_satuan', 'paketsatus.ket_barang')
        ->where('kd_invoice', $transaksis->kd_invoice)
        ->get();
        $checkout_satu = Checkout_satu::join('paketsatus', 'paketsatus.kd_barang', '=', 'checkout_satus.kd_barang')
        ->select('checkout_satus.id', 'checkout_satus.kd_invoice', 'checkout_satus.kd_barang', 'checkout_satus.jumlah_barang', 'checkout_satus.metode_pembayaran', 'checkout_satus.harga_barang as harga_barang_total', 'checkout_satus.harga_antar', 'checkout_satus.harga_total', 'paketsatus.nama_barang', 'paketsatus.harga_barang as harga_barang_satuan', 'paketsatus.ket_barang')
    ...
    public function simpanPesanan(Request $req)
    {
        ...
        {
            $haris = Paketkilo::select('paketkilos.*')
    ...
    public function simpanPelanggan(Request $req)
    {
        ...
        if($cek_username == 1)
        {
            ...
            if($req->ceklevel == 1)
            {
                $pelanggans->ceklevel = "member";
            }else{
                $pelanggans->ceklevel = "non_member";
            }
